...
def analyze_idea(idea_text):
    ...
    # Generate Narrative Summary (~30 words)
    blocker = collisions[0]
    summary_text = f"This concept in the {seed['domain']} sector shows promise but faces high {blocker['type']} friction. With a market readiness of {p_set['factors']['market_adoption']}%, success depends on overcoming the '{blocker['title']}' bottleneck to unlock growth."

    # Generate Dynamic "Next Step" (Wider Picture)
    next_step_map = {
        "Health": "Initiate clinical partnership pilot programs.",
        "FinTech": "Apply for regulatory sandbox approval.",
        "EdTech": "Launch beta curriculum with 5 key institutions.",
        "Productivity": "Develop API integration for enterprise workflows.",
        "E-Commerce": "Secure supply chain logistics contracts.",
        "Social": "Execute influencer-led community expansion.",
        "GreenTech": "Obtain sustainability certification & audit."
    }
    next_step = next_step_map.get(seed['domain'], "Conduct broad-scale market validation surveys.")
...
• EXECUTIVE SUMMARY
  ...
  - Market Readiness: {p_set['factors']['market_adoption']}%
  - Competition Level: {'HIGH' if p_set['c_int'] > 6 else 'LOW'}
  - Pain Point Severity: HIGH

• ENVIRONMENTAL IMPACT ANALYSIS (0-100)
  - Market Adoption: {p_set['factors']['market_adoption']}
  - Regulatory Risk: {p_set['factors']['regulatory_risk']}
  - Societal Trend: {p_set['factors']['societal_trend']}
  - Tech Disruption: {p_set['factors']['tech_disruption']}
  - Supply Chain: {p_set['factors']['supply_chain_risk']}
  - Capital Access: {p_set['factors']['capital_availability']}

• CRITICAL BLOCKER
  - Identified: {blocker['title']}
  - Reason: {blocker['desc']}
  - Severity: {blocker['severity']}/10

• PATH FORWARD & RECOMMENDATIONS
  - Urgent Action (3-5 Days): {blocker['solution']['name']}
    (Impact: {blocker['solution']['impact']}, Time: {blocker['solution']['time']})
  
  - Next Step (Wider Picture):
    {next_step}

• CONCLUSION
  The idea shows strong potential if the '{blocker['title']}' friction is resolved. Focus on the urgent action to validate the core value proposition.
"""
    return report.strip()
